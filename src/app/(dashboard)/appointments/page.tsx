...
<AppointmentTabs />
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-0.5">{appointments.length} encontradas</p>
        </div>
        ...
        {appointments.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-zinc-400">
              {status || dateFrom || dateTo ? "No hay citas que coincidan con los filtros" : "Aún no hay citas registradas"}
            </p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-100">
                <th className="text-left px-5 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wide">Mascota</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wide">Fecha y hora</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wide">Motivo</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wide">Dueño</th>
                <th className="text-left px-5 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wide">Estado</th>
              </tr>
            </thead>
            <tbody>
              {appointments.map((appt) => (
                <tr key={appt.id} className="border-b border-zinc-50 hover:bg-zinc-50 transition-colors cursor-pointer">
                  <td className="px-5 py-3">
                    <Link href={`/appointments/${appt.id}`} className="font-medium text-zinc-700 hover:text-emerald-600 transition-colors">
                      {appt.pet.name}
                    </Link>
                  </td>
                  ...
                  <td className="px-5 py-3 text-zinc-500 max-w-xs truncate">{appt.reason}</td>
                  <td className="px-5 py-3 text-zinc-500">{appt.pet.owner.name}</td>
                  <td className="px-5 py-3">
                    <span className={
